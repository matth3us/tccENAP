...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#load de dados pelo Dropbox}
\CommentTok{#servicos <- readRDS(url('https://www.dropbox.com/s/84hrt2ntrx9k0t0/02_servi%C3%A7os.rds?raw=1'))}
\CommentTok{#viagens <- readRDS('https://www.dropbox.com/s/t17zlg8y8spfmno/01_viagens_2.rds?dl=0')}
\CommentTok{#demog <- readRDS(url('https://www.dropbox.com/s/stsxm0cl98lmnzg/03_demografia.rds?raw=1'))}
\CommentTok{#unids <- readRDS(url('https://www.dropbox.com/s/qaypq8bsoa5gfoa/04_unidadesRFB.rds?raw=1')) %>% }
            \CommentTok{#mutate(ibge_unid = ifelse(is.na(ibge_area), ibge_municipio,  ibge_area)) %>% }
            \CommentTok{#distinct()}

\NormalTok{servicos <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{"02_serviços.rds"}\NormalTok{)}
\NormalTok{viagens <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{'01_viagens.rds'}\NormalTok{)}
\NormalTok{demog <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{'03_demografia.rds'}\NormalTok{)}
\NormalTok{unids <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{'04_unidadesRFB.rds'}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{            }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{ibge_unid =} \KeywordTok{ifelse}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(ibge_area), ibge_municipio,  ibge_area)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{            }\KeywordTok{distinct}\NormalTok{()}


\CommentTok{#unids <- readRDS("./02_ dados/03_ analise/00_unidades_RFB_cod_ibge.rds")}
\CommentTok{#servicos <- readRDS("C:/Users/02741207399/Desktop/Escritório de Processos/Git/Informações extras IBGE/TCC - produtos/02_serviços.rds")}
\CommentTok{#viagens <- readRDS("C:/Users/02741207399/Desktop/Escritório de Processos/Git/Informações extras IBGE/TCC - produtos/01_viagens.rds")}
\CommentTok{#demog <- readRDS("C:/Users/02741207399/Desktop/Escritório de Processos/Git/Informações extras IBGE/TCC - produtos/03_demografia.rds")}
\CommentTok{#unids <- readRDS("C:/Users/02741207399/Desktop/Escritório de Processos/Git/tccENAP/02_ dados/03_ analise/00_unidades_RFB_cod_ibge.rds") %>% }
\CommentTok{#        mutate(ibge_unid = ifelse(is.na(ibge_area), ibge_municipio,  ibge_area)) %>%}
        \CommentTok{#distinct()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{eval02 <-}\StringTok{ }\NormalTok{servicos }\OperatorTok{%>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(publico_alvo }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Pessoa Física"}\NormalTok{))}
...
\StringTok{            }\KeywordTok{filter}\NormalTok{((tipo_atendimento }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Atendimento pela internet"}\NormalTok{, }\StringTok{"Atendimento e-CAC"}\NormalTok{))) }\OperatorTok{%>%}
\StringTok{            }\KeywordTok{filter}\NormalTok{(publico_alvo }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Pessoa Física"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
...
\StringTok{            }\KeywordTok{filter}\NormalTok{(}\OperatorTok{!}\NormalTok{(tipo_atendimento }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Atendimento pela internet"}\NormalTok{, }\StringTok{"Atendimento e-CAC"}\NormalTok{))) }\OperatorTok{%>%}
\StringTok{            }\KeywordTok{filter}\NormalTok{(publico_alvo }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Pessoa Física"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(destino }\OperatorTok{%in%}\StringTok{ }\NormalTok{unids}\OperatorTok{$}\NormalTok{ibge_unid) }\OperatorTok{%>%}\StringTok{ }
...
\includegraphics{trabalho_de_conclusao_final_files/figure-latex/unnamed-chunk-5-1.pdf}
...
\includegraphics{trabalho_de_conclusao_final_files/figure-latex/unnamed-chunk-17-1.pdf}
